...
export const MemoryType = z.enum(['realization', 'pattern'])
export type MemoryType = z.infer<typeof MemoryType>

export const MemorySchema = z.object({
  id: z.string().min(1)
    .describe('Kebab-case slug. Also serves as the filename (memories/{id}.md). Stable — does not change when name changes.'),
  name: z.string().min(1)
    .describe('Short display title. Can be updated freely without breaking references.'),
  type: MemoryType,
  links: z.array(z.string())
    .describe('Ids (slugs) of other memories in this BEP that this memory relates to.'),
  createdAt: z.iso.datetime(),
  updatedAt: z.iso.datetime(),
  data: z.record(z.string(), z.unknown()).optional()
    .describe('Free-form metadata authored by the LLM (e.g. confidence, evidenceCount).'),
}).describe('A project memory generated by an LLM. Stored at memories/{id}.md.')
