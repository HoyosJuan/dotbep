...
export class Bep {
  ...
  generateRuntimeTypes(): string {
    ...
    const hasWorkflows = this._data.workflows.length > 0

    const workflowLines = hasWorkflows
      ? this._data.workflows.map(w => {
          const doc = w.description ? `${w.name} — ${w.description}` : w.name
          return `${jsdoc(doc)}  '${w.id}': never`
        }).join('\n')
      : null

    const workflowIdType = hasWorkflows ? 'WorkflowId' : 'string'
    const triggerReturnType = `Promise<{ trackedAsset: import('@dotbep/core').WorkflowInstance['trackedAsset']; workflowId: ${workflowIdType} }>`
    const triggerSig = `(rawPayload: unknown) => ${triggerReturnType}`

    const triggerLines = this._data.softwares.length
      ? this._data.softwares.map(s => `${jsdoc(s.name)}  '${s.id}': ${triggerSig}`).join('\n')
      : `  [key: string]: ${triggerSig}`
