...
export class Bep {
   ...
   * effects, automations, resolvers, adapters and remoteData. Write the output to
  ...
  generateTypes(): string {
    ...
    const toUnion = (ids: string[]) =>
      ids.length ? ids.map(id => `'${id}'`).join(' | ') : 'never'
    ...
    return [
      ...
      '// ─── ID literals ──────────────────────────────────────────────────────────────',
      '',
      `export type ResolverId   = ${toUnion(this._data.resolvers.map(r => r.id))}`,
      `export type AdapterId    = ${toUnion(this._data.adapters.map(a => a.id))}`,
      `export type EffectId     = ${toUnion(this._data.effects.map(e => e.id))}`,
      `export type AutomationId = ${toUnion(this._data.automations.map(a => a.id))}`,
      '',
      '// ─── Effect payloads ──────────────────────────────────────────────────────────',
      '',
